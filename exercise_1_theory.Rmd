...
rank_dfs <- list(
  ...
  data.frame(parameter = "beta", dataset_id = 1:N_ranks, rank = round(rbeta(N_ranks, shape1 = 3, shape2 = 1) * max_rank)),
  data.frame(parameter = "beta", dataset_id = 1:N_ranks, rank = round(rbeta(N_ranks, shape1 = 8, shape2 = 4) * max_rank))
)
...
rank_dfs <- list(
  data.frame(parameter = "beta", dataset_id = 1:N_ranks, rank = round(
    ifelse(runif(N_ranks) < 0.4, rbeta(N_ranks, shape1 = 15, shape2 = 4), rbeta(N_ranks, shape1 = 30, shape2 = 1))
    * max_rank)),
  data.frame(parameter = "beta", dataset_id = 1:N_ranks, rank = round(rbeta(N_ranks, shape1 = 0.1, shape2 = 0.1) * max_rank)),
  data.frame(parameter = "beta", dataset_id = 1:N_ranks, rank = round(rbeta(N_ranks, shape1 = 6, shape2 = 6) * max_rank))
)
...
### Given the `ecdf_diff` plot for a positive parameter `sigma` below, which is the `ecdf_diff` plot for `log(sigma)`?
...
n_ranks <- 80
max_rank <- 100
ranks_sigma <- data.frame(dataset_id = 1:n_ranks, parameter = "sigma", rank = round(rbeta(n_ranks, 0.6, 0.9) * max_rank))

plot_ecdf_diff(ranks_sigma, max_rank = max_rank) + ggtitle(" ")
```
...
candidates_log_sigma <- list(
  dplyr::mutate(ranks_sigma, parameter = "log(sigma)"),
  data.frame(dataset_id = 1:n_ranks, parameter = "log(sigma)", rank = round(rbeta(n_ranks, 0.5, 0.9) * max_rank)),
  data.frame(dataset_id = 1:n_ranks, parameter = "log(sigma)", rank = round(rbeta(n_ranks, 0.9, 0.9) * max_rank))
)

order <- sample.int(3)

plot_ecdf_diff(candidates_log_sigma[[order[[1]]]], max_rank = max_rank) + ggtitle("A")
plot_ecdf_diff(candidates_log_sigma[[order[[2]]]], max_rank = max_rank) + ggtitle("B")
plot_ecdf_diff(candidates_log_sigma[[order[[3]]]], max_rank = max_rank) + ggtitle("B")
